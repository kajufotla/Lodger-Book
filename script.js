...
class AppUI {
    ...
    static updateFileList(colorClass) {
        ...
        const requiresPreview = (currentTool && currentTool.requiresPreview) || previewTools.includes(window.currentToolId);
    ...
    static async execute(id) {
        ...
        try {
            ...
            // پرانے 10 ٹولز کی پروسیسنگ
                ...
                if (id === 'resizer') { 
                    finalBlob = await this.processResizer(window.activeFiles[0]); 
                    filename = `Resized_Image.${document.getElementById('img-format').value.split('/')[1]}`; 
                } 
                else if (id === 'imgToPdf') { 
                    finalBlob = await this.processImagesToPdf(window.activeFiles); 
                } 
                else if (id === 'merge') { 
                    ...
                    await new Promise(r => setTimeout(r, 10)); 
                    const fileBytes = await window.activeFiles[0].arrayBuffer();
...
// --- CONFIGURATION & PRE-REGISTERED TOOLS ---
const ToolsConfig = {
    merge: { name: "Merge PDF", desc: "Combine & reorder PDFs.", icon: "fa-copy", color: "blue-600", multipleFiles: true, render: (t) => AppUI.renderFileInput(t, true) },
    ...
    imgToPdf: { name: "Image to PDF", desc: "Advanced document creation.", icon: "fa-images", color: "blue-700", multipleFiles: true, render: (t) => AppUI.renderFileInput(t, true, "image/*") + `
        <div class="grid grid-cols-2 gap-3 mt-4">
...
// Load existing tools into the Registry immediately
for (const [key, config] of Object.entries(ToolsConfig)) {
    window.ToolsRegistry[key] = {
        ...config,
        isLoaded: true 
    };
}
