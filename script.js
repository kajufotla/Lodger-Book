...
class ToolManager {
    static async loadTool(toolId) {
        ...
        window.ToolLoadPromises[toolId] = (async () => {
            try {
                // Fetch & Parse Manifest (Error bypassed)
                try {
                    ...
                    console.warn(`[ToolManager] Proceeding without manifest for '${toolId}'.`);
                }
...
class HubManager {
    static async initialize() {
        try {
            ...
            // Fetch offline registry directly from root
        ...
        setTimeout(() => {
            ...
            canvas.innerHTML = `
                <div class="flex items-center space-x-5 mb-8 pb-8 border-b border-slate-100">
                    <div class="w-16 h-16عادل، یہ رہا آپ کا 100% فکسڈ روٹ سکرپٹ۔ 

میں نے اس میں رجسٹری کا پاتھ اور `manifest.json` کی رکاوٹ کو ختم کر دیا ہے تاکہ یہ آپ کی موجودہ فائلوں کے ساتھ ڈائریکٹ چلے۔ پرانے کوڈ کو ڈیلیٹ کر کے یہ مکمل کوڈ پیسٹ کر دیں:
...
class ToolManager {
    static async loadTool(toolId) {
        ...
        window.ToolLoadPromises[toolId] = (async () => {
            try {
                // Fetch & Parse Manifest (Made Optional)
                try {
...
class HubManager {
    static async initialize() {
        try {
            ...
            // FIX: Updated path from './tools/registry.json' to './registry.json'
            const res = await fetch('./registry.json');
            if (!res.ok) throw new Error('Could not find registry.json index.');
            const toolsData = await res.json();
